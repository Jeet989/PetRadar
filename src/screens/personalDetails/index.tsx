...
import {actionUpdateProfile} from '../userProfile/redux/userProfileActions';
...
const PersonalDetails: React.FC<Props> = (props: Props) => {
  ...
  useEffect(() => {
    ...
    check(PERMISSIONS.ANDROID.POST_NOTIFICATIONS).then(res => {
  ...
  const onSubmit = async ({
    ...
    const fineLocation = await requestMultiple([
    ...
    if (
      fineLocation[PERMISSIONS.ANDROID.ACCESS_COARSE_LOCATION] === 'granted' &&
      fineLocation[PERMISSIONS.ANDROID.ACCESS_FINE_LOCATION] === 'granted'
    ) {
      Geolocation.getCurrentPosition(async coords => {
        location.latitude = coords.coords.latitude.toString();
        location.longitude = coords.coords.longitude.toString();
        const onesignalUserId: DeviceState | any =
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <KeyboardAwareScrollView showsVerticalScrollIndicator={false}>
        <Text style={styles.createProfile}>Personal Details</Text>
        ...
        <View style={styles.contentView}>
          ...
          <View>
            <Formik
              initialValues={{
                firstName: reduxData?.auth?.firstName
                  ? reduxData?.auth?.firstName
                  : '',
                lastName: reduxData?.auth?.lastName
                  ? reduxData?.auth?.lastName
                  : '',
                email: reduxData?.auth?.email ? reduxData?.auth?.email : '',
                address: reduxData?.auth?.address
                  ? reduxData?.auth?.address
                  : '',
                city: reduxData?.auth?.city ? reduxData?.auth?.city : '',
                pinCode: reduxData?.auth?.pinCode
                  ? reduxData?.auth?.pinCode
                  : '',
                phoneNumber: reduxData?.auth?.phoneNumber?.toString()
                  ? reduxData?.auth?.phoneNumber?.toString()
                  : '',
              }}
              onSubmit={onSubmit}

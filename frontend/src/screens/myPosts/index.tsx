...
import { deletePostAPI, showMyPostAPI } from '../../utils/API/routes';
import { store } from '../../redux/store';

interface Props { }

const MyPosts: React.FC<Props> = (props: Props) => {
            ...
            .then((statuses) => {
                if (statuses[PERMISSIONS.ANDROID.ACCESS_COARSE_LOCATION] === 'granted' && statuses[PERMISSIONS.ANDROID.ACCESS_FINE_LOCATION] === 'granted') {
                    GeoLocation.getCurrentPosition(async (position) => {
                        axiosInstance.get(showMyPostAPI + '?userId=' + store.getState().auth.userId)
                            .then(res => {
                                setIsLoading(false)
                                setRefreshing(false)
            ...
            <View style={styles.rednerView}>
                <View style={styles.cardHeader}>
                    <Image
                        ...
                        ImageComponent={() => <FastImage
                            source={{ uri: reduxData.auth.profilePic ? reduxData.auth.profilePic : 'https://1.bp.blogspot.com/-kK7Fxm7U9o0/YN0bSIwSLvI/AAAAAAAACFk/aF4EI7XU_ashruTzTIpifBfNzb4thUivACLcBGAsYHQ/s1280/222.jpg', priority: FastImage.priority.high }}
                            style={styles.profilePic}
                        />}
                    />

                    <View style={styles.userDetailsView}>
                        <Text style={styles.username}>{reduxData.auth.firstName + " " + reduxData?.auth?.lastName}</Text>
                    </View>
                ...
                <View>
                    ...
                    <Image
                        // @ts-ignore
                        ImageComponent={() => <FastImage
                            source={{ uri: item.item.imageUrl, priority: FastImage.priority.high }}
